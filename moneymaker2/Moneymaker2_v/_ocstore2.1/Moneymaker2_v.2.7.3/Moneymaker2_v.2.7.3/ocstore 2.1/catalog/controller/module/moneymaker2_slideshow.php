<?php
class ControllerModuleMoneymaker2Slideshow extends Controller {
	public function index($setting) {
		...
		$data['moneymaker2_slideshow_image_width'] = $setting['width'];
		$data['moneymaker2_slideshow_image_height'] = $setting['height'];
		...
		if (!empty($banners_settings)&&$setting['width']>0&&$setting['height']>0) {
      $banners_proportion = round($setting['width']/$setting['height']);
			foreach ($banners_settings as $key => $value) {
				$data['banners_settings'][] = array(
					...
					'image_sm' => is_file(DIR_IMAGE . $value['image'])&&($data['fullwidth']||$data['fullwidth_bottom']) ? $this->model_tool_image->resize($value['image'], 990, 990/$banners_proportion) : $this->model_tool_image->resize('no_image.png', $setting['width'], $setting['height']),
					'image_xs' => is_file(DIR_IMAGE . $value['image'])&&($data['fullwidth']||$data['fullwidth_bottom']) ? $this->model_tool_image->resize($value['image'], 766, 766/$banners_proportion) : $this->model_tool_image->resize('no_image.png', $setting['width'], $setting['height']),
					'image_xxs' => is_file(DIR_IMAGE . $value['image'])&&($data['fullwidth']||$data['fullwidth_bottom']) ? $this->model_tool_image->resize($value['image'], 560, 560/$banners_proportion) : $this->model_tool_image->resize('no_image.png', $setting['width'], $setting['height']),
					'image_xxxs' => is_file(DIR_IMAGE . $value['image'])&&($data['fullwidth']||$data['fullwidth_bottom']) ? $this->model_tool_image->resize($value['image'], 360, 360/$banners_proportion) : $this->model_tool_image->resize('no_image.png', $setting['width'], $setting['height']),
				);
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/module/moneymaker2_slideshow.tpl')) {
			return $this->load->view($this->config->get('config_template') . '/template/module/moneymaker2_slideshow.tpl', $data);
		} else {
			return $this->load->view('default/template/module/moneymaker2_slideshow.tpl', $data);
		}
	}
}