...
class ControllerModuleMoneymaker2Slideshow extends Controller {
	...
	public function index() {
		$this->load->language('module/moneymaker2_slideshow');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			if(isset($this->request->post['apply']) and $this->request->post['apply'])
				$this->response->redirect($this->url->link('module/moneymaker2_slideshow', 'token=' . $this->session->data['token'] . '&module_id=' . $this->request->get['module_id'], 'SSL'));
			else
		...
		if (!isset($this->request->get['module_id'])) {
			$data['breadcrumbs'][] = array(
				...
				'href' => $this->url->link('module/moneymaker2_slideshow', 'token=' . $this->session->data['token'], 'SSL')
			);
		} else {
			$data['breadcrumbs'][] = array(
				'text' => $this->language->get('heading_title'),
				'href' => $this->url->link('module/moneymaker2_slideshow', 'token=' . $this->session->data['token'] . '&module_id=' . $this->request->get['module_id'], 'SSL')
			);
		}

		if (!isset($this->request->get['module_id'])) {
			$data['action'] = $this->url->link('module/moneymaker2_slideshow', 'token=' . $this->session->data['token'], 'SSL');
		} else {
			$data['action'] = $this->url->link('module/moneymaker2_slideshow', 'token=' . $this->session->data['token'] . '&module_id=' . $this->request->get['module_id'], 'SSL');
		}
...
/*        if (isset($this->request->post['banners_settings'])) {
		...
		$this->response->setOutput($this->load->view('module/moneymaker2_slideshow.tpl', $data));
	}

	protected function validate() {
		if (!$this->user->hasPermission('modify', 'module/moneymaker2_slideshow')) {
