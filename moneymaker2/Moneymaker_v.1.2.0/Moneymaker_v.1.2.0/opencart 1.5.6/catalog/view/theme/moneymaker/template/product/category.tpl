...
<div id="content">
  <div class="row">
    <div class="col-sm-12">
      ...
      <?php if ($this->config->get('mmr_advantages_enabled')&&$this->config->get('mmr_advantages')&&$this->config->get('mmr_advantages_display_category')) { ?>
      ...
      <div class="row">
        <div class="col-sm-12 pos-9">
          ...
          <?php if ($this->config->get('mmr_catalog_category_description_hidden')!=1) { ?>
          <div class="row category-description">
            <div class="col-xs-12">
              ...
              <?php } ?>
              <?php if ($description) { ?>
                <?php if ($this->config->get('mmr_catalog_category_description_spoiled')!=1||(mb_strlen(strip_tags(html_entity_decode($description, ENT_QUOTES, 'UTF-8')))<=30+$this->config->get('mmr_catalog_category_description_spoiler_limit')&&$this->config->get('mmr_catalog_category_description_spoiled'))) { ?>
                <?php echo $description; ?>
                <?php } else { ?>
                <?php echo utf8_substr(strip_tags(html_entity_decode($description, ENT_QUOTES, 'UTF-8')), 0, $this->config->get('mmr_catalog_category_description_spoiler_limit'))."<span class='description-expand'>.. </span>"; ?><a class="description-expand" href="javascript:void(0);" onclick="$('.category-description .collapse').fadeIn('normal');$('.description-expand').hide();" title="<?php echo $this->language->get('button_view'); ?>"><i class="fa fa-caret-down"></i></a><span class="collapse"><?php echo utf8_substr(strip_tags(html_entity_decode($description, ENT_QUOTES, 'UTF-8')), $this->config->get('mmr_catalog_category_description_spoiler_limit')); ?></span>
                <?php } ?>
              <?php } ?>
            </div>
          </div>
          ...
          <div class="row categories">
            <div class="col-xs-12">
              <?php if ($this->config->get('mmr_catalog_category_refine_hidden')!=1) { ?>
              ...
              <div>
                ...
                <div class="category-layout">
                  ...
                  <?php if ($category['thumb']&&$this->config->get('mmr_catalog_categories_images_hidden')!=1) { ?>
          ...
          <div class="row products product-<?php if ($this->config->get('mmr_catalog_layout')) { ?>list<?php } else { ?>grid<?php } ?>" itemscope itemtype="http://schema.org/ItemList">
            <div>
                ...
                <div class="product-block <?php if ($this->config->get('mmr_catalog_grid_details')=='Always') { ?>product-block-detailed<?php } ?>">
                  <div class="image">
                    <?php if ($this->config->get('mmr_stickers_enabled')) { ?>
                      ...
                      <?php foreach ($mmr_stickers as $key => $mmr_sticker) { ?>
                        <?php if ($mmr_sticker['enabled']) { ?>
                          <?php if ( (($key==1)&&$product['rating']) || ((($key==2)||($key==7)||($key==8)||($key==9))&&isset($product[$mmr_sticker['limit']])&&$product[$mmr_sticker['limit']]) || (($key==3)&&$product['special']) || (($key==4)&&isset($product['viewed'])&&$product['viewed']>=$mmr_sticker['limit']) || (($key==5)&&isset($product['date_available'])&&(round((strtotime(date("Y-m-d"))-strtotime($product['date_available']))/86400))<=$mmr_sticker['limit']) || (($key==6)&&$product['rating']&&$product['rating']>=$mmr_sticker['limit']) ) { ?>
                            <div class="sticker sticker-<?php echo $key; ?>"<?php if ($key==1&&$product['rating']) { ?> itemprop="aggregateRating" itemscope itemtype="http://schema.org/AggregateRating"<?php }  ?>>
                              ...
                              <?php if ($key==1) { ?>
                              <span itemprop = "ratingValue"><?php echo $product['rating']; ?></span><small>/5</small>
                              <?php } else { ?>
                              ...
                              <?php if ($key==1) { ?>
                                ...
                                <div class="sticker-caption-bottom"><?php echo $this->language->get('button_reviews'); ?>: <span itemprop = "reviewCount"><?php echo $product['review_count']; ?></span></div>
                              <?php }  ?>
                              </div>
                            </div>
                            <?php if ($key>1) break; ?>
                      ...
                      <?php } ?>
                    <?php } ?>
                    <a href="<?php echo $product['href']; ?>" itemprop="url"><img src="<?php echo $product['thumb']; ?>" alt="<?php echo $product['name']; ?>" title="<?php echo $product['name']; ?>" class="img-responsive img-thumbnail-transparent" itemprop="image" /></a>
                  </div>
                  <div class="name">
...
<?php echo $footer; ?>
<?php if ($this->config->get('mmr_catalog_category_description_hidden')!=1&&$this->config->get('mmr_catalog_category_description_dropped')) { ?>
