      ...
      <div>
        ...
        <div class="product-layout">
          <div class="product-block product-block-detailed">
            ...
            <?php if ($this->config->get('mmr_common_bestsellers_stock_enabled')||$this->config->get('mmr_common_bestsellers_product_code_enabled')||$product['description']||$product['attribute_groups']||$product['meta_description']) { ?>
            <div class="description">
              <?php if ($this->config->get('mmr_common_bestsellers_stock_enabled')||$this->config->get('mmr_common_bestsellers_product_code_enabled')) { ?>
              <div class="additional-fields">
                <?php if ($this->config->get('mmr_common_bestsellers_stock_enabled')) { ?><span class="stock<?php if ($product['quantity']<=0) { ?> outofstock<?php } ?>"><?php if (!$this->config->get('mmr_common_stock_availability_hidden')) { ?><b><?php echo $this->language->get('text_stock'); ?></b> <?php } ?><?php echo $product['stock']; ?></span><br /><?php } ?>
                <?php if ($this->config->get('mmr_common_bestsellers_product_code_enabled')) { ?><?php $mmr_common_product_code_caption = $this->config->get('mmr_common_product_code_caption'); ?><?php echo html_entity_decode($mmr_common_product_code_caption[$this->config->get('config_language_id')], ENT_QUOTES, 'UTF-8'); ?> <?php $mmr_common_product_code_field=$this->config->get('mmr_common_product_code_field'); if ($product[$mmr_common_product_code_field]) { ?><?php echo $product[$mmr_common_product_code_field]; ?><?php } ?><?php } ?>
              </div>
              <?php } ?>
