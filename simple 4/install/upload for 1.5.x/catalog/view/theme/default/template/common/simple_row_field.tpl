<tr class="row-<?php echo $id ?>">
  ...
  <td class="simplecheckout-table-form-right">
    ...
    <?php } elseif ($type == 'radio') { ?>
      <?php foreach ($values as $info) { ?>
        <label><input type="radio" name="<?php echo $name ?>" id="<?php echo $id ?>_<?php echo $info['id'] ?>" value="<?php echo $info['id'] ?>" <?php echo $value == $info['id'] ? 'checked="checked"' : '' ?> <?php echo $reload ? 'data-onchange="reloadAll"' : 'data-reload-payment-form="true"'?>><?php echo $info['text'] ?></label>
      <?php } ?>
    <?php } elseif ($type == 'checkbox') { ?>
      <?php foreach ($values as $info) { ?>
        <label><input type="checkbox" name="<?php echo $name ?>[]" id="<?php echo $id ?>_<?php echo $info['id'] ?>" value="<?php echo $info['id'] ?>" <?php echo in_array($info['id'], $value) ? 'checked="checked"' : '' ?> <?php echo $reload ? 'data-onchange="reloadAll"' : 'data-reload-payment-form="true"'?>><?php echo $info['text'] ?></label>
      <?php } ?>
    ...
    <?php } elseif ($type == 'file') { ?>
      ...
      <input type="hidden" name="<?php echo $name ?>" id="<?php echo $id ?>" value="<?php echo $value ?>">
    <?php } else { ?>
      <input type="<?php echo ($type == 'date' || $type == 'time' || $type == 'datetime') ? 'text' : $type ?>" <?php echo $type == 'password' ? 'data-validate-on="keyup"' : '' ?> name="<?php echo $name ?>" id="<?php echo $id ?>" value="<?php echo $value ?>" placeholder="<?php echo $placeholder ?>" <?php echo $attrs ?> <?php echo $reload ? 'data-onchange="reloadAll"' : 'data-reload-payment-form="true"'?>>
    <?php } ?>
    <?php if (!empty($rules)) { ?>
